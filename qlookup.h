...
//// Some hacks can be found at this page:
//// http://graphics.stanford.edu/~seander/bithacks.html

//// Also see http://home.comcast.net/~danheisman/Articles/Articles.html
...
const U64 MaskFile[8]  ={0b0000000100000001000000010000000100000001000000010000000100000001,
...
const U64 ClearFile[8] ={0b1111111011111110111111101111111011111110111111101111111011111110,
...
//  bb += bb >>  8;  //put count of each 16 bits into their lowest 8 bits
//  bb += bb >> 16;  //put count of each 32 bits into their lowest 8 bits
//  bb += bb >> 32;  //put count of each 64 bits into their lowest 8 bits
//  return bb & 0x7f;
//}

//int PopCount3(U64 bb){
//  bb -= (bb >> 1) & m1;              //put count of each 2 bits into those 2 bits
//  bb = (bb & m2) + ((bb >> 2) & m2); //put count of each 4 bits into those 4 bits
//  bb = (bb + (bb >> 4)) & m4;        //put count of each 8 bits into those 8 bits
...
//int getSetBits(U64 bb, int* mask){
//  int i=0;
//  while(bb){
//    mask[i] = ffsll(bb);
//    bb >> mask[i];
...
//    bitCount[bitRef] = getSetBits(mask,setBitsInMask);
//    variationCount = (int)(1L << bitCount[bitRef]);
//    for(i=0; i<variationCount; i++){
//      occupancyVariation[bitRef].push_back(0);
//      // find bits set in index "i" and map them to bits in the 64 bit "occupancyVariation"
//      getSetBits(i,setBitsInIndex); // an array of integers showing which bits are set
...
//void generateMagicNumbers(const bool isRook, U64* magicNumbers, int* magicNumberShifts){
...
//  random_device rd;
//  mt19937_64 e2(rd());
//  uniform_int_distribution<U64> dist(llround(pow(2,61)),llround(pow(2,62)));
//  for(int n=0; n<10; ++n) cout << dist(e2)<< ", " ;
...
//  for(bitRef=0; bitRef<=63; bitRef++){
//    const int bitCount = PopCount1(isRook ? occupancyMaskRook[bitRef] : occupancyMaskBishop[bitRef]);
//    variationCount = (int)(1L << bitCount);
//    bool fail;
//    U64 usedBy[(int)(1L << bitCount)];

//    int attempts = 0;

//    do{
//      magicNumber = dist(e2) & dist(e2) & dist(e2); // generate a random number with not many bits set
...
//      }
//    } while(fail);
//    magicNumbers[bitRef] = magicNumber;
//    magicNumberShifts[bitRef] = (64-bitCount);
//  }
//  return;
//}
...
//  for(bitRef=0; bitRef<=63; bitRef++){
//    bitCount = isRook ? PopCount1(occupancyMaskRook[bitRef]) : PopCount1(occupancyMaskBishop[bitRef]);
